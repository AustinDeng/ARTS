...
> 块大小是一个重要的参数设计。我们选择64MB比大多数文件系统块都要大。每一个块的复制都作为linux普通文件存储在块服务器上而且只有在需要时才扩展。惰性空间分配避免了因为内部分裂导致的空间浪费。

> 一个大的块大小提供了一些重要的优势。首先，他减小了客户端与master的交互因为同一个块的读写只需要向master询问一次块位置信息。这种减小对我们的工作负载具有重大意义因为应用大多数顺序的读写文件。甚至对于小的随机读，客户端能从容的一个大小为几个TB大小工作集的块位置信息。第二，因为在一个大的块上，一个客户端很可能执行许多操作在给定的块上，他可以通过对块服务器保持持续的TCP连接减少网络花费时间。第三，他减小了存储在master上元数据的大小。这让我们得以将元数据保存在内存中，这也变相的带来很多好处我们将在2.6.1部分提到。

> 另一方面，大的块大小，甚至是惰性分配有他的缺点。一个小的文件包含一些小块甚至只有一个。如果许多客户端持续访问相同的文件话，块服务器存储这些块将变得很棘手。在实践中，一个个热点并不是一个主要问题，因为我们的应用大多顺序的读大块文件。

> 但是当GFS第一次被一个批量队列系统(一个可执行被作为一个单个块文件写到GFS，在上百个机器上同时开始)使用时，hot spots出现了被成百上千个同时请求时很少有块服务器存储这个可执行文件。我们通过用更高的复制元素存储这次执行和让批处理队列应用的开始时间错开来解决这个问题。一个潜在的长久解决方案是让客户端从处于同样情形的服务器上读数据。

# 2.6 Metadat

> master主机存储三种类型的主要元数据:文件和块空间，文件和块之间的映射，每一个复制块的地址。所有的元数据存放在master的内存中。前两种类型的变动被持久记录到存储在master本地硬盘上的操作日志和远程机器上。使用日志让我们可以简单的可靠地不必在冒主机事件冲突的风险情形下更新主机状态。master不必持久的存储块位置信息。取而代之的是，在每次启动和当块服务器加入集群时他向每一个块服务器询问节点。
