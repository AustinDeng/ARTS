...
> 一个大的块大小提供了一些重要的优势。首先，他减小了客户端与master的交互因为同一个块的读写只需要向master询问一次块位置信息。这种减小对我们的工作负载具有重大意义因为应用大多数顺序的读写文件。甚至对于小的随机读，客户端能从容的一个大小为几个TB大小工作集的块位置信息。第二，因为在一个大的块上，一个客户端很可能执行许多操作在给定的块上，他可以通过对块服务器保持持续的TCP连接减少网络花费时间。
